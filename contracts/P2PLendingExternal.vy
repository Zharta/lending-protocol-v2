...
@dev It facilitates peer-to-peer lending using NFTs as collateral.
      ...
      - Accepting loan offers and locking NFTs in an escrow contract as collateral
...
escrow_address: public(immutable(address))
...
def __init__(
    ...
    @param _max_protocol_upfront_fee The maximum percentage (bps) of the principal paid to the protocol at origination.
    @param _escrow_address The address of the escrow contract.
    """
...
def _store_punk(_wallet: address, _collateralAddress: address, _tokenId: uint256):
    ...
    assert offer.onlySellTo == empty(address) or offer.onlySellTo == escrow_address, "collateral buying not authorized"
...
def _store_erc721(_wallet: address, _collateralAddress: address, _tokenId: uint256):
    extcall IERC721(_collateralAddress).safeTransferFrom(_wallet, escrow_address, _tokenId, b"")
...
def _transfer_collateral(wallet: address, collateral_contract: address, token_id: uint256):
    ...
    if self._is_punk(collateral_contract):
        self._store_punk(escrow_address, collateral_contract, token_id)
...
def _store_collateral(wallet: address, collateral_contract: address, token_id: uint256):
    ...
    if self._is_punk(collateral_contract):
        ...
        self._transfer_punk(escrow_address, collateral_contract, token_id)

    else:
        assert self._erc721_owner(collateral_contract, token_id) == wallet, "collateral not owned by wallet"
        assert self._is_erc721_approved(wallet, collateral_contract, token_id), "transfer is not approved"
