...
def test_replace_loan_reverts_if_loan_defaulted(p2p_nfts_eth, ongoing_loan_bayc, now, offer_bayc2):
    ...
    with boa.reverts("loan defaulted"):
        p2p_nfts_eth.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=ongoing_loan_bayc.borrower)
...
def test_replace_loan_reverts_if_offer_not_signed_by_lender(p2p_nfts_eth, borrower_key, ongoing_loan_bayc, offer_bayc2):
    ...
    with boa.reverts("offer not signed by lender"):
        p2p_nfts_eth.replace_loan(ongoing_loan_bayc, signed_offer, 0, 0, ZERO_ADDRESS, sender=ongoing_loan_bayc.borrower)
...
def test_replace_loan_reverts_if_origination_fee_exceeds_principal(
    p2p_nfts_eth, ongoing_loan_bayc, now, lender, lender_key, bayc
):
    token_id = 1
    offer = Offer(
        principal=1000,
...
def test_replace_loan_reverts_if_collateral_contract_mismatch(
    p2p_nfts_eth, p2p_control, ongoing_loan_bayc, now, lender, lender_key, bayc, weth
):
    token_id = 1
    principal = 1000
...
def test_replace_loan_reverts_if_funds_not_sent(p2p_nfts_eth, ongoing_loan_bayc, weth, offer_bayc2):
    ...
    deposit_and_approve(weth, principal, new_lender, p2p_nfts_eth.address)

    with boa.reverts("invalid sent value"):
        p2p_nfts_eth.replace_loan(
            ongoing_loan_bayc, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle - 1
        )
...
def test_replace_loan_reverts_if_collateral_locked(p2p_nfts_eth, p2p_control, ongoing_loan_bayc, now, offer_bayc2, weth):
    ...
    deposit_and_approve(weth, principal, lender, p2p_nfts_eth.address)
    ...
    with boa.reverts("collateral locked"):
        p2p_nfts_eth.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)
...
def test_replace_loan(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2, now, bayc, weth):
    ...
    amount_to_settle = ongoing_loan_bayc.amount + ongoing_loan_bayc.interest
    deposit_and_approve(
        weth, principal - offer.origination_fee_amount + offer.broker_upfront_fee_amount, lender, p2p_nfts_eth.address
    )

    loan_id = p2p_nfts_eth.replace_loan(
        ongoing_loan_bayc, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle
    )
...
def test_replace_loan_works_with_proxy(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2, now, bayc, weth, p2p_nfts_proxy):
    ...
    p2p_nfts_eth.set_proxy_authorization(p2p_nfts_proxy, True, sender=p2p_nfts_eth.owner())
    loan_id = p2p_nfts_proxy.replace_loan(
        ongoing_loan_bayc, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle
    )
...
def test_replace_loan_transfers_principal_to_borrower(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2, weth):
    ...
    amount_to_settle = ongoing_loan_bayc.amount + ongoing_loan_bayc.interest
    deposit_and_approve(
        weth, principal - offer.origination_fee_amount + offer.broker_upfront_fee_amount, new_lender, p2p_nfts_eth.address
    )
...
def test_replace_loan_transfers_origination_fee_to_lender(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2, weth):
    ...
    deposit_and_approve(weth, principal - origination_fee + offer.broker_upfront_fee_amount, new_lender, p2p_nfts_eth.address)

    p2p_nfts_eth.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)

    assert (
        boa.env.get_balance(new_lender)
...
def test_replace_loan_pays_borrower_if_amount_to_settle_negative(
    p2p_nfts_eth, ongoing_loan_bayc, offer_bayc, weth, lender, lender_key
):
    loan = ongoing_loan_bayc
    ...
    p2p_nfts_eth.replace_loan(
        ongoing_loan_bayc, sign_offer(offer, lender_key, p2p_nfts_eth.address), 0, 0, ZERO_ADDRESS, sender=borrower
    )
...
def test_replace_loan_pays_broker_fees(p2p_nfts_eth, ongoing_loan_bayc, offer_bayc2, weth):
    ...
    deposit_and_approve(
        weth, new_principal - offer.origination_fee_amount + offer.broker_upfront_fee_amount, new_lender, p2p_nfts_eth.address
    )

    p2p_nfts_eth.replace_loan(ongoing_loan_bayc, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)

    assert (
        boa.env.get_balance(broker_address)
...
def test_replace_loan_prorata_logs_event(p2p_nfts_eth, ongoing_loan_prorata, weth, now, offer_bayc2):
    ...
    boa.env.time_travel(seconds=actual_duration)
    loan_id = p2p_nfts_eth.replace_loan(loan, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)
...
def test_replace_loan_prorata_pays_lender(p2p_nfts_eth, ongoing_loan_prorata, weth, offer_bayc2):
    ...
    print(f"{amount_to_settle=}")
    boa.env.time_travel(seconds=actual_duration)
    p2p_nfts_eth.replace_loan(loan, offer_bayc2, 0, 0, ZERO_ADDRESS, sender=borrower, value=amount_to_settle)
...
def test_replace_loan_settles_amounts(  # noqa: PLR0914
    ...
    bayc.approve(p2p_nfts_eth.address, token_id, sender=borrower)
    deposit_and_approve(
        weth, principal_loan1 - origination_fee + offer.broker_upfront_fee_amount, lender, p2p_nfts_eth.address
    )
    ...
    if lender != lender2:
        deposit_and_approve(weth, -new_lender_delta, lender2, p2p_nfts_eth.address)
    elif current_lender_delta + new_lender_delta < 0:
        deposit_and_approve(weth, -(current_lender_delta + new_lender_delta), lender, p2p_nfts_eth.address)

    boa.env.time_travel(seconds=actual_duration)
    loan2_id = p2p_nfts_eth.replace_loan(
        loan1,
        signed_offer2,
        borrower_broker_upfront_fee,
        borrower_broker_settlement_fee,
