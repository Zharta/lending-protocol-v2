...
def test_create_loan_reverts_if_offer_has_invalid_signature(p2p_nfts_usdc, borrower, now, lender, lender_key, usdc):
    ...
    invalid_offers = [
        ...
        replace_namedtuple_field(offer, token_ids=[2]),
        replace_namedtuple_field(offer, token_ids=[*offer.token_ids, 2]),
...
def test_create_loan_reverts_if_token_id_below_offer_range(p2p_nfts_usdc, borrower, now, lender, lender_key, bayc, usdc):
    ...
    offer = Offer(
        ...
        token_ids=[token_id + 1, token_id + 2],
        expiration=now + 100,
        lender=lender,
        pro_rata=False,
    )
...
def test_create_loan_reverts_if_token_id_not_in_list(p2p_nfts_usdc, borrower, now, lender, lender_key, bayc, usdc):
    token_id = 1
    offer = Offer(
        principal=1000,
        interest=100,
...
def test_create_loan_reverts_if_settlement_fees_exceed_principal(
    p2p_nfts_usdc, borrower, now, lender, lender_key, bayc, usdc, protocol_fee, lender_broker_fee
):
    token_id = 1
...
def test_create_loan_reverts_if_collateral_not_approved_punks(
    p2p_nfts_usdc, borrower, now, lender, lender_key, cryptopunks, usdc
):
    token_id = 1
    principal = 1000
    offer = Offer(
...
def test_create_loan_reverts_if_broker_fee_too_high(p2p_nfts_usdc, borrower, now, lender, lender_key, bayc, usdc):
    token_id = 1
    offer = Offer(
        principal=1000,
        interest=100,
...
def test_create_loan_reverts_if_borrower_broker_fee_too_high(p2p_nfts_usdc, borrower, now, lender, lender_key, bayc, usdc):
    token_id = 1
    offer = Offer(
        principal=1000,
        interest=100,
